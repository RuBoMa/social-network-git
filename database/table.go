...
if _, err := db.Exec(insertUsersQuery); err != nil {
	...
	insertPostsQuery := `
    INSERT INTO posts (title, user_id, content, image_path, privacy, created_at, updated_at) 
    SELECT 'Welcome to the forum', 'This is the first post!', 1, '', 'public', datetime('now'), datetime('now')
    WHERE NOT EXISTS (
        SELECT 1 FROM posts WHERE title = 'Welcome to the forum'
    );
`
	_, err := db.Exec(insertPostsQuery)
	if err != nil {
