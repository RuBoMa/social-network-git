...
func StoreSession(sessionID string, userID int, expirationTime time.Time) error {
	...
	_, err := db.Exec(`
    INSERT INTO Sessions (user_id, session_token, updated_at, expires_at, creater_at) 
    VALUES (?, ?, ?, ?, ?)`,
		userID,
...
func GetSessionFromDB(sessionID string) (int, error) {
	var userID int
	err := db.QueryRow("SELECT user_id FROM Sessions WHERE id = ? AND status = 'active'", sessionID).Scan(&userID)
	if err != nil {
